  ...
  return (
    <div className="bg-dark-card p-8 rounded-4xl flex items-center shadow-8 gap-x-8 mb-6">
      <div className="gap-y-4 flex flex-col justify-center">
        <Chevron onClick={() => onClick(true)} className="cursor-pointer" />
        <p className="text-center font-semibold text-2xl select-none">{comment.comment_rating}</p>
        <Chevron onClick={() => onClick(false)} className="rotate-180 cursor-pointer" />
      </div>
